...
# Filter by group all types of ASDM versus TDB - results_asdm_tdb_combined#

results_asdm_tdb_combined <- rma(yi = Cohens_d, sei = SE_d,
                             data = data[data$Treatment %in% c("ASDM") & data$Control == "TDB",], 
                             method = "REML")
summary(results_asdm_tdb_combined) # Summarise results

# Define a specific subset for ASDM vs. TDB
subset_asdm_tdb <- data[data$Treatment %in% c("ASDM") & data$Control == "TDB",]

# Filter data for each outcome and run separate meta-analyses
results_cognitive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdb[subset_asdm_tdb$Cognitive == 1,], method="REML")
results_adaptive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdb[subset_asdm_tdb$Adaptive.functioning == 1,], method="REML")
results_social <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdb[subset_asdm_tdb$Social.Communication == 1,], method="REML")
results_linguistic <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdb[subset_asdm_tdb$Linguistic == 1,], method="REML")
...
# Meta-regression with outcomes as moderators - how different outcomes affect the effect sizes for ASDM vs TDB
results_meta_reg <- rma(yi=Cohens_d, sei=SE_d, mods = ~ Cognitive + Adaptive.functioning + Social.Communication + Linguistic, data=subset_asdm_tdb, method="REML")
...
subset_asdm_tdb$StudyLabel <- paste(subset_asdm_tdb$ID, subset_asdm_tdb$year, subset_asdm_tdb$Outcome.term, sep="        ")
...
cognitive_data <- subset_asdm_tdb[subset_asdm_tdb$Cognitive == 1,]
...
results_Adaptive.functioning <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdb[subset_asdm_tdb$Adaptive.functioning == 1,], method="REML")

# Filter the data for cognitive outcomes
Adaptive.functioning_data <- subset_asdm_tdb[subset_asdm_tdb$Adaptive.functioning == 1,]
...
# Running the meta-analysis
results_Social.Communication <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdb[subset_asdm_tdb$Social.Communication == 1,], method="REML")

# Filter the data for cognitive outcomes
Social.Communication_data <- subset_asdm_tdb[subset_asdm_tdb$Social.Communication == 1,]
...
# Filter the data for cognitive outcomes
Linguistic_data <- subset_asdm_tdb[subset_asdm_tdb$Linguistic == 1,]
