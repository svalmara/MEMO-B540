...
# Filter by group all types of ASDB versus ODDB - results_asdb_combined#

results_asdb_oddb_combined <- rma(yi = Cohens_d, sei = SE_d,
                             data = data[data$Treatment %in% c("ASDB", "ASDBH", "ASDBSEQ", "ASDBSIM", "ASDBL") & data$Control == "ODDB",], 
                             method = "REML")
summary(results_asdb_oddb_combined) # Summarise results

forest(results_asdb_oddb_combined, main="Results Combined - ASDB-ODDB")

# Define a specific subset for ASDB vs. ODDB
subset_asdb_oddb <- data[data$Treatment %in% c("ASDB", "ASDBH", "ASDBSEQ", "ASDBSIM", "ASDBL") & data$Control == "ODDB",]

# Filter data for each outcome and run separate meta-analyses
results_cognitive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_oddb[subset_asdb_oddb$Cognitive == 1,], method="REML")
results_adaptive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_oddb[subset_asdb_oddb$Adaptive.functioning == 1,], method="REML")
results_social <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_oddb[subset_asdb_oddb$Social.Communication == 1,], method="REML")
results_linguistic <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_oddb[subset_asdb_oddb$Linguistic == 1,], method="REML")
...
results_meta_reg <- rma(yi=Cohens_d, sei=SE_d, mods = ~ Cognitive + Adaptive.functioning + Social.Communication + Linguistic, data=subset_asdb_oddb, method="REML")
...
subset_asdb_oddb$StudyLabel <- paste(subset_asdb_oddb$ID, subset_asdb_oddb$year, subset_asdb_oddb$Outcome.term, sep="        ")
...
cognitive_data <- subset_asdb_oddb[subset_asdb_oddb$Cognitive == 1,]
...
results_Adaptive.functioning <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_oddb[subset_asdb_oddb$Adaptive.functioning == 1,], method="REML")

# Filter the data for cognitive outcomes
Adaptive.functioning_data <- subset_asdb_oddb[subset_asdb_oddb$Adaptive.functioning == 1,]
...
# Running the meta-analysis
results_Social.Communication <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_oddb[subset_asdb_oddb$Social.Communication == 1,], method="REML")

# Filter the data for cognitive outcomes
Social.Communication_data <- subset_asdb_oddb[subset_asdb_oddb$Social.Communication == 1,]
...
# Filter the data for cognitive outcomes
Linguistic_data <- subset_asdb_oddb[subset_asdb_oddb$Linguistic == 1,]
