...
results_asdb_combined <- rma(yi = Cohens_d, sei = SE_d,
                             data = data[data$Treatment %in% c("ASDBSIM") & data$Control == "ASDM",], 
                             method = "REML")
summary(results_asdb_combined) # Summarise results

# Define a specific subset for ASDB vs. asdm
subset_asdbh_asdm <- data[data$Treatment %in% c("ASDBSIM") & data$Control == "ASDM",]

# Filter data for each outcome and run separate meta-analyses
results_cognitive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdbh_asdm[subset_asdbh_asdm$Cognitive == 1,], method="REML")
results_adaptive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdbh_asdm[subset_asdbh_asdm$Adaptive.functioning == 1,], method="REML")
results_social <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdbh_asdm[subset_asdbh_asdm$Social.Communication == 1,], method="REML")
results_linguistic <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdbh_asdm[subset_asdbh_asdm$Linguistic == 1,], method="REML")
...
results_meta_reg <- rma(yi=Cohens_d, sei=SE_d, mods = ~ Cognitive + Adaptive.functioning + Social.Communication + Linguistic, data=subset_asdbh_asdm, method="REML")
...
subset_asdbh_asdm$StudyLabel <- paste(subset_asdbh_asdm$ID, subset_asdbh_asdm$year, subset_asdbh_asdm$Outcome.term, sep=" ")
...
cognitive_data <- subset_asdbh_asdm[subset_asdbh_asdm$Cognitive == 1,]
...
results_Adaptive.functioning <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdbh_asdm[subset_asdbh_asdm$Adaptive.functioning == 1,], method="REML")

# Filter the data for cognitive outcomes
Adaptive.functioning_data <- subset_asdbh_asdm[subset_asdbh_asdm$Adaptive.functioning == 1,]
...
# Running the meta-analysis
results_Social.Communication <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdbh_asdm[subset_asdbh_asdm$Social.Communication == 1,], method="REML")

# Filter the data for cognitive outcomes
Social.Communication_data <- subset_asdbh_asdm[subset_asdbh_asdm$Social.Communication == 1,]
...
# Filter the data for cognitive outcomes
Linguistic_data <- subset_asdbh_asdm[subset_asdbh_asdm$Linguistic == 1,]
