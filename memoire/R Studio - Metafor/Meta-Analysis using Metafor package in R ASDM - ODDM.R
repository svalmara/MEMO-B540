...
# Filter by group all types of ASDM versus ODDM - results_asdm_oddm_combined#

results_asdm_oddm_combined <- rma(yi = Cohens_d, sei = SE_d,
                             data = data[data$Treatment %in% c("ASDB", "ASDBH", "ASDBSEQ", "ASDBSIM", "ASDBL") & data$Control == "ODDB",], 
                             method = "REML")
summary(results_asdm_oddm_combined) # Summarise results

forest(results_asdm_oddm_combined, main="Results Combined - ASDB-ODDB")

# Define a specific subset for ASDB vs. ODDB
subset_asdm_oddm <- data[data$Treatment %in% c("ASDB", "ASDBH", "ASDBSEQ", "ASDBSIM", "ASDBL") & data$Control == "ODDB",]

# Filter data for each outcome and run separate meta-analyses
results_cognitive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_oddm[subset_asdm_oddm$Cognitive == 1,], method="REML")
results_adaptive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_oddm[subset_asdm_oddm$Adaptive.functioning == 1,], method="REML")
results_social <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_oddm[subset_asdm_oddm$Social.Communication == 1,], method="REML")
results_linguistic <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_oddm[subset_asdm_oddm$Linguistic == 1,], method="REML")
...
results_meta_reg <- rma(yi=Cohens_d, sei=SE_d, mods = ~ Cognitive + Adaptive.functioning + Social.Communication + Linguistic, data=subset_asdm_oddm, method="REML")
...
subset_asdm_oddm$StudyLabel <- paste(subset_asdm_oddm$ID, subset_asdm_oddm$year, subset_asdm_oddm$Outcome.term, sep="        ")
...
cognitive_data <- subset_asdm_oddm[subset_asdm_oddm$Cognitive == 1,]
...
results_Adaptive.functioning <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_oddm[subset_asdm_oddm$Adaptive.functioning == 1,], method="REML")

# Filter the data for cognitive outcomes
Adaptive.functioning_data <- subset_asdm_oddm[subset_asdm_oddm$Adaptive.functioning == 1,]
...
# Running the meta-analysis
results_Social.Communication <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_oddm[subset_asdm_oddm$Social.Communication == 1,], method="REML")

# Filter the data for cognitive outcomes
Social.Communication_data <- subset_asdm_oddm[subset_asdm_oddm$Social.Communication == 1,]
...
# Filter the data for cognitive outcomes
Linguistic_data <- subset_asdm_oddm[subset_asdm_oddm$Linguistic == 1,]
