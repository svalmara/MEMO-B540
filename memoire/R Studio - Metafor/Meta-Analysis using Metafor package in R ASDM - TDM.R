...
# Filter by group all types of ASDM versus TDM - results_asdm_combined#

results_asdm_combined <- rma(yi = Cohens_d, sei = SE_d,
                             data = data[data$Treatment %in% c("ASDM") & data$Control == "TDM",], 
                             method = "REML")
summary(results_asdm_combined) # Summarise results

# Define a specific subset for ASDB vs. ASDM
subset_asdm_tdm <- data[data$Treatment %in% c("ASDM") & data$Control == "TDM",]

# Filter data for each outcome and run separate meta-analyses
results_cognitive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdm[subset_asdm_tdm$Cognitive == 1,], method="REML")
results_adaptive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdm[subset_asdm_tdm$Adaptive.functioning == 1,], method="REML")
results_social <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdm[subset_asdm_tdm$Social.Communication == 1,], method="REML")
results_linguistic <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdm[subset_asdm_tdm$Linguistic == 1,], method="REML")
...
results_meta_reg <- rma(yi=Cohens_d, sei=SE_d, mods = ~ Cognitive + Adaptive.functioning + Social.Communication + Linguistic, data=subset_asdm_tdm, method="REML")
...
subset_asdm_tdm$StudyLabel <- paste(subset_asdm_tdm$ID, subset_asdm_tdm$year, subset_asdm_tdm$Outcome.term, sep="        ")
...
cognitive_data <- subset_asdm_tdm[subset_asdm_tdm$Cognitive == 1,]
...
results_Adaptive.functioning <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdm[subset_asdm_tdm$Adaptive.functioning == 1,], method="REML")

# Filter the data for cognitive outcomes
Adaptive.functioning_data <- subset_asdm_tdm[subset_asdm_tdm$Adaptive.functioning == 1,]
...
# Running the meta-analysis
results_Social.Communication <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdm_tdm[subset_asdm_tdm$Social.Communication == 1,], method="REML")

# Filter the data for cognitive outcomes
Social.Communication_data <- subset_asdm_tdm[subset_asdm_tdm$Social.Communication == 1,]
...
# Filter the data for cognitive outcomes
Linguistic_data <- subset_asdm_tdm[subset_asdm_tdm$Linguistic == 1,]
