...
# Filter by group all types of ASDB versus tdb - results_asdb_combined#

results_asdb_combined <- rma(yi = Cohens_d, sei = SE_d,
                             data = data[data$Treatment %in% c("ASDB", "ASDBH", "ASDBSEQ", "ASDBSIM", "ASDBL") & data$Control == "TDB",], 
                             method = "REML")
summary(results_asdb_combined) # Summarise results

# Define a specific subset for ASDB vs. TDB
subset_asdb_tdb <- data[data$Treatment %in% c("ASDB", "ASDBH", "ASDBSEQ", "ASDBSIM", "ASDBL") & data$Control == "TDB",]

# Filter data for each outcome and run separate meta-analyses
results_cognitive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_tdb[subset_asdb_tdb$Cognitive == 1,], method="REML")
results_adaptive <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_tdb[subset_asdb_tdb$Adaptive.functioning == 1,], method="REML")
results_social <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_tdb[subset_asdb_tdb$Social.Communication == 1,], method="REML")
results_linguistic <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_tdb[subset_asdb_tdb$Linguistic == 1,], method="REML")
...
# Meta-regression with outcomes as moderators - how different outcomes affect the effect sizes for ASDB vs tdb
results_meta_reg <- rma(yi=Cohens_d, sei=SE_d, mods = ~ Cognitive + Adaptive.functioning + Social.Communication + Linguistic, data=subset_asdb_tdb, method="REML")
...
subset_asdb_tdb$StudyLabel <- paste(subset_asdb_tdb$ID, subset_asdb_tdb$year, subset_asdb_tdb$Outcome.term, sep="        ")
...
cognitive_data <- subset_asdb_tdb[subset_asdb_tdb$Cognitive == 1,]
...
results_Adaptive.functioning <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_tdb[subset_asdb_tdb$Adaptive.functioning == 1,], method="REML")

# Filter the data for cognitive outcomes
Adaptive.functioning_data <- subset_asdb_tdb[subset_asdb_tdb$Adaptive.functioning == 1,]
...
# Running the meta-analysis
results_Social.Communication <- rma(yi=Cohens_d, sei=SE_d, data=subset_asdb_tdb[subset_asdb_tdb$Social.Communication == 1,], method="REML")

# Filter the data for cognitive outcomes
Social.Communication_data <- subset_asdb_tdb[subset_asdb_tdb$Social.Communication == 1,]
...
# Filter the data for cognitive outcomes
Linguistic_data <- subset_asdb_tdb[subset_asdb_tdb$Linguistic == 1,]
